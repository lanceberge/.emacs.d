...
(use-package desktop ; save sessions to a file
  ...
  :straight (:type built-in)
  :custom
  (desktop-path (list (expand-file-name "desktop/" my/etc-dir)))
  :config
  (desktop-save-mode 1)) ; TODO manual?
#+END_SRC
** Keybindings
#+BEGIN_SRC emacs-lisp :results none
...
(use-package evil-magit ; evil bindings for magit TODO in evil-collection
  :after magit
  :custom
...
(use-package org
  ...
  ;; (org-agenda-files (append (directory-files-recursively "~/school" "\.org$") '("~/org")))
  ...
  (org-todo-keywords '((sequence "TODO(t)" "TODAY(T)" "IN PROGRESS(p!)"
...
(use-package exec-path-from-shell ; Use system $PATH variable for eshell, commands, etc.
  :hook (after-init . (lambda () (setq exec-path-from-shell-arguments nil)
